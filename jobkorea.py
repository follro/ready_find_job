...
# DataFrame 선언
df = pd.DataFrame( columns = ['회사로고', '회사정보'])
...
for i in trange(20):                       # for i in trange(pages): 를 통해 전체 페이지 탐색 가능. 시간이 오래걸려 일단 20페이지만 탐색
    ...
    while j < 20:
        ...
        company_infos = soup2.find('div', class_='tbcol coInfo').find_all('dl', class_='tbList')

        if company_logo:
            df.loc[20 * page_no + j] = [company_logo[0]['src'],
                                        company_infos[j].text]
            
            print(company_logo[0]['src'])
            print(company_infos[j].text)
        else:
